...
function AuthorTable({authors, showAddedAtColumn, showCheckboxes, selectedEntities, onToggleRow, genderOptions}) {
	...
	if (authors.length) {
				...
				<Table striped>
					<thead>
						<tr>
							...
							<th>{translate('common:dateOfBirth')}</th>
							<th>{translate('common:dateOfDeath')}</th>
							{
		...
		tableContent = <span>{translate('entityDisplay.table.noAuthors')}</span>;
	}
	return (
		<div>
			<h2>{translate('common:entityType.author_plural', {defaultValue: 'Authors'})}</h2>
			{tableContent}
		</div>
	);
}
