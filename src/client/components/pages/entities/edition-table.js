	...
	if (editions.length) {
			...
			<React.Fragment>
				<Table striped>
					<thead>
						<tr>
							...
							{showAuthorCreditsColumn && <th>{translate('common:authorCredits')}</th>}
							<th>{translate('common:format')}</th>
							<th>{translate('common:isbn')}</th>
							<th>{translate('common:releaseDate')}</th>
							{
				...
				{showAdd &&
					...
					>
						...
						{`  ${translate('common:button.addEdition')}`}
					</Button>
				}
			</React.Fragment>
		);
		...
		tableContent = (
			<React.Fragment>
				...
				<span>
					<h4>{translate('entityDisplay.table.noEditionsYet')}</h4>
					<p>
		...
		tableContent = <span>{translate('entityDisplay.table.noEditions')}</span>;
	}
	return (
		<div>
			<h2>{translate('common:entityType.edition_plural', {defaultValue: 'Editions'})}</h2>
			{tableContent}
		</div>
	);
}
