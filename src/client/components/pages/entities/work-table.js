...
function WorkTable({entity, showAddedAtColumn, works, showAdd, showCheckboxes, selectedEntities, onToggleRow}) {
	...
	if (works.length) {
				...
				<Table striped>
					<thead>
						<tr>
							...
							{showAuthors && <th>{translate('common:entityType.author', {defaultValue: 'Author'})}</th>}
							<th>{translate('common:languages', {defaultValue: 'Languages'})}</th>
							<th>{translate('common:type')}</th>
							{
								showAddedAtColumn ? <th>{translate('common:addedAt')}</th> : null
							}
						</tr>
					</thead>
					<tbody>
			...
			<React.Fragment>
				<span className="margin-right-2 float-left">
					...
					>
						...
						<br/>
						{translate('common:button.addWork')}
					</Button>
				</span>
				<span>
					<h4>{translate('entityDisplay.table.noWorksYet', {defaultValue: 'There are no Works yet!'})}</h4>
					<p>
						{translate('entityDisplay.table.helpCompleteBB')}
						<br/>
					</p>
					<br/>
					<small>
		...
		tableContent = <span>{translate('entityDisplay.table.noWorks')}</span>;
	}
	return (
		<div>
			<h2>{translate('common:entityType.work_plural', {defaultValue: 'Works'})}</h2>
			{tableContent}
		</div>
	);
}
